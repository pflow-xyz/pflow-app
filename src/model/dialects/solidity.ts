...
abstract contract MyModelContract is Metamodel {
    ...
    enum Properties {place0, place1, SIZE}
    enum Actions {txn0, txn1, txn2, txn3, HALT}

    int256[] public state = new int256[](uint8(Properties.SIZE));

    constructor() {
        cell("place0", 1, 3, Model.Position(1, 2));
        cell("place1", 0, 0, Model.Position(4, 3));


        func("txn0", uint8(Properties.SIZE), uint8(0), uint8(Roles.Roles.DEFAULT), Model.Position(0, 1));
        func("txn1", uint8(Properties.SIZE), uint8(1), uint8(Roles.Roles.DEFAULT), Model.Position(2, 1));
        func("txn2", uint8(Properties.SIZE), uint8(2), uint8(Roles.Roles.DEFAULT), Model.Position(0, 3));
        func("txn3", uint8(Properties.SIZE), uint8(3), uint8(Roles.Roles.DEFAULT), Model.Position(2, 3));
        ...
        arrow(1, transitions[3], places[1]);
