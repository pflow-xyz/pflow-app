import React, { useEffect, useState } from 'react';
           ...
           function readModelSource(model: string): ModelData | Error {
               const newModel = JSON.parse(model) as ModelData;
               for (const place of Object.values(newModel.places)) {
                   if (isNaN(place.x) || isNaN(place.y) || place.x <= 0 || place.y <= 0) {
                       return new Error("Invalid place coordinates");
                   }
               }
               for (const transition of Object.values(newModel.transitions)) {
                   if (isNaN(transition.x) || isNaN(transition.y) || transition.x <= 0 || transition.y <= 0) {
                       return new Error("Invalid transition coordinates");
                   }
               }
                return newModel;
           }
           ...
           function postMessageToModel(data: any, origin: string = '*') {
               const svg = document.getElementById('svgObject') as HTMLObjectElement;
               if (svg) {
                   svg.addEventListener('load', () => {
                       if (svg.contentWindow) {
                           svg.contentWindow.postMessage(data, origin);
                       }
                   });
               }
           }
           ...
           export default function App() {
               const [modelState, setModelState] = useState<Model>(getModel());
               const [editorHeight, setEditorHeight] = useState(Math.max(window.innerHeight - 680, 300));

               useEffect(() => {
                   const handleResize = () => {
                       const height = Math.max(window.innerHeight - 725, 300);
               ...
               useEffect(() => {
                   ...
                   postMessageToModel({ type: 'setModel', model: modelState });
                   postMessageToModel({ type: 'resize', width, height: 600 });
               }, [modelState]);
                   ...
                   <React.Fragment>
                       <svg width="100%" height="630px" xmlns="http://www.w3.org/2000/svg">
                           ...
                           <foreignObject height="630" width="100%" x="0" y="0">
                               <object id="svgObject" key={modelState.toMinUrl()} type="image/svg+xml" data="./model.svg">
                       ...
                       <CodeEditor
                           ...
                           onChange={(evt) => {
                               try {
                                   var newModel = readModelSource(evt.target.value);
                                   if (newModel instanceof Error) {
                                     //console.error(newModel);
                                     return;
                                   } else {
                                       newModel = newModel as ModelData;
                                   }
                                   const m = new Model(newModel);
                                   const newUrl = m.toMinUrl();
                                   if (newUrl !== modelState.toMinUrl()) {
                                       setModelState(m);
                                       postMessageToModel({ type: 'setModel', model: m });
                                   }
                               } catch (e) {
                       ...
                       <div id="footer">
                           <a href="https://github.com/pflow-xyz/pflow-app">
                               <svg viewBox="0 0 24 24" id="GitHubIcon" transform="translate(0, 4)">
